...
# ------------------------------------------------------------------
# RUN FULL PIPELINE + TIMER + VISUALIZATION
# ------------------------------------------------------------------
def run_bfs_on_map(start, goal, max_hole_area: int = 60,
    ...
    # Check start/goal cell values
    print("Start cell value (0=free,1=occ,-1=unk):", grid_for_bfs[start])
    print("Goal  cell value (0=free,1=occ,-1=unk):", grid_for_bfs[goal])

    # Build color image for visualization
    display_grid = np.zeros((*grid.shape, 3), dtype=np.uint8)
    display_grid[grid == 1] = [0, 0, 0]           # black: obstacle
    display_grid[grid == 0] = [255, 255, 255]     # white: free
    display_grid[grid == -1] = [128, 128, 128]    # grey: unknown
    ...
    black_patch = mpatches.Patch(color='black', label='Obstacle (1)')
