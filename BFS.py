...
def make_occupancy_grid(gray: np.ndarray, negate: int,
    ...
    if negate == 0:
        occ_prob = (255.0 - gray.astype(np.float32)) / 255.0
    else:
...
def inflate_obstacles(grid: np.ndarray, robot_radius: float, resolution: float) -> np.ndarray:
    ...
    cell_radius = int(np.ceil(robot_radius / resolution))
    if cell_radius <= 0:
        return inflated
    ...
    for r, c in occ_indices:
        ...
        r_max = min(h - 1, r + cell_radius)
        c_min = max(0, c - cell_radius)
        c_max = min(w - 1, c + cell_radius)
        for rr in range(r_min, r_max + 1):
            for cc in range(c_min, c_max + 1):
           
                if grid[rr, cc] == 0:
                    
                    if (rr - r) ** 2 + (cc - c) ** 2 <= cell_radius ** 2:
                        inflated[rr, cc] = 1
...
def run_bfs_on_map(start, goal, max_hole_area: int = 60,
    ...
    grid = inflate_obstacles(grid, robot_radius=ROBOT_RADIUS, resolution=meta["resolution"])
    ...
    print("Start cell value (0=free,1=occ,-1=unk):", grid_for_bfs[start])
    print("Goal  cell value (0=free,1=occ,-1=unk):", grid_for_bfs[goal])

    display_grid = np.zeros((*grid.shape, 3), dtype=np.uint8)
    display_grid[grid == 1] = [0, 0, 0]           # obstacle (inflated)
    display_grid[grid == 0] = [255, 255, 255]     # free
    display_grid[grid == -1] = [128, 128, 128]    # unknown
    ...
    if path:
        ...
        plt.plot(xs, ys, color='blue', linewidth=2, label="BFS Path")

    plt.scatter(start[1], start[0], color='red', s=120, label="Start")
    plt.scatter(goal[1], goal[0], color='green', s=120, label="Goal")

    plt.title("BFS Path on Occupancy Grid (Inflated Obstacles)")
    plt.axis("off")

    black_patch = mpatches.Patch(color='black', label='Obstacle (1) inflated')
    white_patch = mpatches.Patch(color='white', label='Free (0)')
